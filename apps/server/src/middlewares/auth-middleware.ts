...
export const protect = createMiddleware(async (c: Context, next: Next) => {
  ...
  if (!token) {
    return c.json({ message: "Please login to access this resource" }, HttpStatusCodes.UNAUTHORIZED);
  }
...
export const protectAndIsAdmin = createMiddleware(async (c: Context, next: Next) => {
  ...
  try {
    ...
    if (!user.isAdmin) {
      return c.json({ message: "Not authorized as an admin!" }, HttpStatusCodes.UNAUTHORIZED);
    }
    ...
    // Log the error and return an unauthorized response for invalid token
    c.var.logger.error(err);
    return c.json({ message: "Invalid token" }, HttpStatusCodes.UNAUTHORIZED);
  }
});
